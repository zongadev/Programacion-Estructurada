...
int main(){
    int datosFuente[T] = { 25, -25, 45, 345, 512, 345, 
		                 654, 287, 567, 124 };   // 4*T=40
    int datosDestino[T] = {0};
                                
	FILE* fp =fopen("prueba.dat", "w+b");        // ABRIR  el archivo de (W+b):escritura / lectura Binario

    printf( "Escribiendo en el archivo las cantidades:\n\n" );
    imprimirVector(datosFuente);                 // Imprimir el vactor para ver el pantalla lo que se cargar� en e archivo
...
//int escribirArchivo(FILE* fp,  int* datos)
   /** escribe en un archivo binario (pasado el puntero del descriptor por par�metro)
...
int leerArchivo(FILE* fp, int datos[T]){
    /** leer en un archivo binario (pasado el puntero del descriptor por par�metro)
